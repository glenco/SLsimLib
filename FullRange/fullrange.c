...
*/
  ...
  for(i=0; i < Nsources ;++i){
	  ...
	  j=0;
	  if(i==0){  // find an source position that makes 4 images
		  do{
	  ...
	  for(m=1;m<=1;++m){
		  ...
		  if(m==0){
			  ...
			  //lens->stars_N = 40000;  // This is a cheat.  Needs to match number in paramfile
		 ...
		 //       ;r_source_phys /= pow(10,1.0/40.0), ++refresh ){
		  for(r_source_phys = 1.0e-9, refresh = 0;r_source_phys >= 0.99999e-9
			        ;r_source_phys /= pow(10,1.0/40.0), ++refresh ){
