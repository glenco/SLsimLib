...
/** \mainpage
 ...
 Where "paramfile" is a string containing the path and file name of your parameter file.  A sample parameter
 file with all allowed parameters can be generated by InputParams params = InputParams::sample();  This 
 can be printed to a usable parameters file with InputParams::PrintToFile().
 ...
 The mean functions used for image finding are find_images_kist(), find_images_microlens() and map_images().
 These take a Grid and a Lens structure.  The found images are then stored in an array of ImageInfo structures.
 
   <em> OUTPUT: </em>
 
 The Grid has some direct output functions for making maps that do not depend on sources (kappa, gamma, etc.).  
 The ImageInfo structure has some information about the images found and contains a linked list of all the points 
 within the image.  Each point contains information about the kappa, gamma, source position, time-delay and in 
 some cases the the surface brightness at that point.
 
   <em> PREPROCESSOR FLAGS: </em>

The currently used preprocessor flags are:
 
 N_THREADS          -  set to the maximum number of threads to be used
 GSL                -  set if GSL library is available.  This enables some halo model calculations in CosmoLib.
 ENABLE_FITS        -  set if cfitsio and CCfits libraries are available.  This enable i/o with fits images.
 ENABLE_FFTW        -  set if FFTW library is available.  This is required for using density maps as lenses.
 ENABLE_CLANG       -  set if using clang compiler.  This utilizes some of the most up to date features in the c++11 standard.
 _OPENMP            -  enables openMP multi-threading, but this is used in only one place and should be considered obsolete.
 
 These can be set using the cmake -D command. (example: cmake .. -DN_THREADS=16, cmake .. -L will give the current values.)
   </pre>
 */
