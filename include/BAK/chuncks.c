...
int main(void) {
	double *data_out;
        char filename[0xFFF];
	hsize_t i, j, nrows = 0, ncols=7;
        FILE *pFile = fopen("/scratch1/marcos/Weak_lensing_project/LSS_catalog/file.h5", "r");

        if (getFilename(pFile, filename)) {
          printf("EL FICHERO ES: %s\n", filename);
          H5std_string H5filename(filename);
	  data_out = common_scan_blockH5(H5filename, "/dset", 1000, ncols, 0,  &nrows);
        }
        else {
          printf("ERROR!!!!!!");
        }
