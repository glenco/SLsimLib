...
int main(int arg,char **argv){
  ...
  double range,source[2],r_source,center[2],linkinglength,rmin,rmax,*times,x[2];
...
/*   exit(0); */
  ...
  for(i=0;i<Ngrid_sources*Ngrid_sources;++i){
      ...
      time(&t3);
      find_images(source_centers[i].x,r_source,linkinglength,s_tree,i_tree,&Nimages
    		  ,imageinfo,&NImagePoints,0,True,verbose);
      time(&t4);
