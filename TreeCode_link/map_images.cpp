...
/*
	if(FindCenter){
		...
		for(i=0;i<Nsources;++i){
			...
			centers[i][0] += source->getTheta()[0];
			centers[i][1] += source->getTheta()[1];
		...
		ImageFinding::find_images_kist(source->getTheta(),xmin,grid,Nimages
			  ,imageinfo,NimageMax,&Nimagepoints,initial_size,true,0,false,true);
	}
*/

	//if(oldr==0) oldr=source->source_r_out;
	//if((oldy[0]==source->getTheta()[0])*(oldy[1]==source->getTheta()[1])*(oldr > source->source_r_out)) initial_size=oldr;
...
/*	}else{
		...
		for( ; rtemp >= r_source*Ngrid_block ;rtemp /= Ngrid_block ){
			...
			do{
				...
				do{
					...
					y[0] = point->image->x[0] - source->getTheta()[0];
					y[1] = point->image->x[1] - source->getTheta()[1];
			...
			}while(IF_routines::refine_grid_kist(grid,imageinfo,*Nimages,rtemp*mumin/Ngrid_block,2,kappa_off,true,dummy_pnt));
	...
	/*/********** test lines **********************
	PointsWithinKist_iter(grid->s_tree,source->getTheta(),0,source->source_r_out,imageinfo->imagekist);
  ...
  bool touches_edge;
	...
	***/
	...
	oldy[0] = source->getTheta()[0];
	oldy[1] = source->getTheta()[1];
...
void ImageFinding::map_images_fixedgrid(
                ...
                ,std::vector<ImageInfo> &imageinfo   /// information on each image
                ,PosType xmax            /// Maximum size of source on image plane.  The entire source must be within this distance from
                                        ///  source->getTheta()[].  Decreasing it will make the code run faster.  Making xmax much bigger than
  ...
  grid->s_tree->PointsWithinKist(source->getTheta().x, xmax, imageinfo[0].imagekist, 0);
  ...
  for(imageinfo[0].imagekist->MoveToTop();!(imageinfo[0].imagekist->OffBottom());){
    ...
    x[0] = imageinfo[0].imagekist->getCurrent()->x[0] - source->getTheta()[0];
    x[1] = imageinfo[0].imagekist->getCurrent()->x[1] - source->getTheta()[1];
 ...
 *
 */
int ImageFinding::IF_routines::refine_grid_on_image(Lens *lens,Source *source,GridHndl grid,PosType maxflux
  ...
  for(i=0,Ncells=0;i<(*Nimages);++i){
	  ...
	  if(imageinfo[i].uniform_mag == unchecked){
		  ImageFinding::IF_routines::UniformMagCheck(&imageinfo[i]);
...
void ImageFinding::IF_routines::check_sb_add(Source *source,ImageInfo *imageinfo,Point *i_points,PosType maxflux,unsigned long &Nold,int &number_of_refined){
	if(i_points != NULL){
		// link new points into image kist and calculate surface brightnesses
...
bool ImageFinding::IF_routines::RefinePoint2(Point *point,TreeHndl i_tree,PosType image_area,PosType total_area,PosType maxflux
...
bool ImageFinding::IF_routines::RefinePoint_sb(Point *point,TreeHndl i_tree,PosType image_area,PosType total_area
...
// refinement criterion based on difference in surface brightness
bool ImageFinding::IF_routines::RefinePoint_smallsize(Point *point,TreeHndl i_tree,PosType image_area,PosType total_area
 ...
 *
bool IF_routines::RefinePoint(Point *point,TreeHndl i_tree,PosType image_area,PosType total_area
		,ExitCriterion criterion,PosType res_target,Kist<Point> * nearest){
 ...
 * distorted.
 */
void ImageFinding::IF_routines::UniformMagCheck(ImageInfo *imageinfo){
