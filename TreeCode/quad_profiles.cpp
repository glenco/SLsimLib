...
double QuadTreeNFW::alpha_h(double r2,HaloStructure &par){
	...
	if(r < par.Rmax){
		...
		y = r/par.rscale;
		b*= gfunction(y);
		//b*= gfunctionRmax(par.Rmax/par.rscale,y);
	}
...
double QuadTreeNFW::kappa_h(double r2,HaloStructure &par){
	...
	y = r/par.rscale;
	b*= ffunction(y);
	//b*= ffunctionRmax(par.Rmax/par.rscale,y);
...
double QuadTreeNFW::gamma_h(double r2,HaloStructure &par){
	...
	if(r < par.Rmax){
		...
		y = r/par.rscale;
		gt *= g2function(y);
		//gt *= g2functionRmax(par.Rmax/par.rscale,y);
	}
...
double QuadTreeNFW::g2functionRmax(double rm,double x){
	...
	if(x<1.0){ans -= 1/xx*(4.0/x/x+2.0/(x*x-1.0))*(atanh(xx/rm)-atanh(xx*rmx/rm)); return ans;}
